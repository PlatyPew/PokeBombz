...
public class TitleScene implements Screen {
    ...
    private static final int PLAY2_BUTTON_X =
        GameInfo.WIDTH / 2 - PLAY2_BUTTON_WIDTH / 2 - PLAY2_BUTTON_WIDTH;
    ...
    private static final int LEVEL_BUTTON_X =
        GameInfo.WIDTH / 2 - LEVEL_BUTTON_WIDTH / 2 - LEVEL_BUTTON_WIDTH;
    ;
    ...
    public TitleScene(GameMain game) {
        ...
        animation = GifDecoder.loadGIFAnimation(Animation.PlayMode.LOOP,
                                                Gdx.files.internal("screen/bg.gif").read());
    ...
    public void render(float delta) {
        ...
        if (bye)
            batch.draw(exitlogo, GameInfo.WIDTH / 2 - 1010 / 2, TITLELOGO_Y);

        // 2Player button
        if (active && Gdx.input.getX() < PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH &&
            Gdx.input.getX() > PLAY2_BUTTON_X - PLAY2_BUTTON_WIDTH &&
            GameInfo.HEIGHT - Gdx.input.getY() < PLAY2_BUTTON_HEIGHT + PLAY2_BUTTON_Y &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y) {
            if (active)
                batch.draw(play2ButtonActive, PLAY2_BUTTON_X, PLAY2_BUTTON_Y);
            ...
            if (active)
                batch.draw(play2ButtonInactive, PLAY2_BUTTON_X, PLAY2_BUTTON_Y);
        }

        // 3Player button
        if (active && Gdx.input.getX() < PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH * 2 &&
            Gdx.input.getX() > PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH &&
            GameInfo.HEIGHT - Gdx.input.getY() < PLAY2_BUTTON_HEIGHT + PLAY2_BUTTON_Y &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y) {
            if (active)
                batch.draw(play3ButtonActive, PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH, PLAY2_BUTTON_Y);
            if (Gdx.input.justTouched()) {
                active = false;
                numP = 3;
                System.out.println(numP);
            ...
            if (active)
                batch.draw(play3ButtonInactive, PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH,
                           PLAY2_BUTTON_Y);
        }

        // 4Player button
        if (active && Gdx.input.getX() < PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH * 3 &&
            Gdx.input.getX() > PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH * 2 &&
            GameInfo.HEIGHT - Gdx.input.getY() < PLAY2_BUTTON_HEIGHT + PLAY2_BUTTON_Y &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y) {
            if (active)
                batch.draw(play4ButtonActive, PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH * 2,
                           PLAY2_BUTTON_Y);
            if (Gdx.input.justTouched()) {
                active = false;
                numP = 4;
            ...
            if (active)
                batch.draw(play4ButtonInactive, PLAY2_BUTTON_X + PLAY2_BUTTON_WIDTH * 2,
                           PLAY2_BUTTON_Y);
        }

        // Level 1 Button
        if (!active && Gdx.input.getX() < LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH &&
            Gdx.input.getX() > LEVEL_BUTTON_X - LEVEL_BUTTON_WIDTH &&
            GameInfo.HEIGHT - Gdx.input.getY() < LEVEL_BUTTON_HEIGHT + PLAY2_BUTTON_Y - 50 &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y - 50) {
            if (!active)
                batch.draw(levelOneButtonActive, LEVEL_BUTTON_X, PLAY2_BUTTON_Y - 50);
            if (Gdx.input.justTouched()) {
                this.dispose();
                menuMusic.dispose();
            ...
            if (!active)
                batch.draw(levelOneButtonInactive, LEVEL_BUTTON_X, PLAY2_BUTTON_Y - 50);
        }

        // Level 2 Button
        if (!active && Gdx.input.getX() < LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH * 2 &&
            Gdx.input.getX() > LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH &&
            GameInfo.HEIGHT - Gdx.input.getY() < PLAY2_BUTTON_HEIGHT + PLAY2_BUTTON_Y - 50 &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y - 50) {
            if (!active)
                batch.draw(levelTwoButtonActive, LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH,
                           PLAY2_BUTTON_Y - 50);
            if (Gdx.input.justTouched()) {
                this.dispose();
                menuMusic.dispose();
            ...
            if (!active)
                batch.draw(levelTwoButtonInactive, LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH,
                           PLAY2_BUTTON_Y - 50);
        }

        // Level 3 Button
        if (!active && Gdx.input.getX() < LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH * 3 &&
            Gdx.input.getX() > LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH * 2 &&
            GameInfo.HEIGHT - Gdx.input.getY() < PLAY2_BUTTON_HEIGHT + PLAY2_BUTTON_Y - 50 &&
            GameInfo.HEIGHT - Gdx.input.getY() > PLAY2_BUTTON_Y - 50) {
            if (!active)
                batch.draw(levelThreeButtonActive, LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH * 2,
                           PLAY2_BUTTON_Y - 50);
            if (Gdx.input.justTouched()) {
                this.dispose();
                menuMusic.dispose();
            ...
            if (!active)
                batch.draw(levelThreeButtonInactive, LEVEL_BUTTON_X + LEVEL_BUTTON_WIDTH * 2,
                           PLAY2_BUTTON_Y - 50);
        }

        // Exit button
        int ex = GameInfo.WIDTH / 2 - EXIT_BUTTON_WIDTH / 2;
        if (active && Gdx.input.getX() < ex + EXIT_BUTTON_WIDTH && Gdx.input.getX() > ex &&
            GameInfo.HEIGHT - Gdx.input.getY() <
                EXIT_BUTTON_HEIGHT + EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 3 &&
            GameInfo.HEIGHT - Gdx.input.getY() > EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 3) {
            if (active)
                batch.draw(exitButtonActive, ex, EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 3);
            if (Gdx.input.justTouched()) {
                bye = true;
            ...
            if (active)
                batch.draw(exitButtonInactive, GameInfo.WIDTH / 2 - EXIT_BUTTON_WIDTH / 2,
                           EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 3);
        }
        // Back button
        int bk = GameInfo.WIDTH / 2 - BACK_BUTTON_WIDTH / 2;
        if (!active && Gdx.input.getX() < bk + BACK_BUTTON_WIDTH && Gdx.input.getX() > ex &&
            GameInfo.HEIGHT - Gdx.input.getY() <
                BACK_BUTTON_HEIGHT + EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 2 &&
            GameInfo.HEIGHT - Gdx.input.getY() > EXIT_BUTTON_Y - PLAY2_BUTTON_HEIGHT * 2) {
            if (!active)
                batch.draw(backButtonActive, ex, EXIT_BUTTON_Y - BACK_BUTTON_HEIGHT * 2);
            if (Gdx.input.justTouched()) {
                active = true;
            ...
            if (!active)
                batch.draw(backButtonInactive, ex, EXIT_BUTTON_Y - BACK_BUTTON_HEIGHT * 2);
        }
